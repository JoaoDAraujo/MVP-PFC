...
- Armazenamento: O banco de dados Firebase Firestore armazena o hash resultante e o segredo do 2FA de forma estruturada.
...
- Fluxo de Validação: A validação do 2FA ocorre estritamente após a autenticação primária (e-mail e senha). O sistema utiliza um estado de sessão temporário (pending2faUserId) para impedir que o usuário acesse o dashboard antes de validar o token.
...
- Na tela de cadastro há um bloco de termos de uso expansível (accordion) contendo a política de privacidade completa, incluindo finalidade da coleta, segurança dos dados, direitos do titular e base legal.
- O cadastro só é concluído se o usuário marcar explicitamente a caixa de aceite. O campo lgpdAcceptedAt registra no Firestore a data e hora exatas do aceite, gerando evidência auditável.
...
├── controllers/
│   ├── authController.ts  → lógica de signup, login, 2FA, logout e Google OAuth
│   └── recoveryController.ts → recuperação de senha em 3 etapas via e-mail
├── routes/
